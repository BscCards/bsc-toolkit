<template>
  <div class="vanity-wallet-page">
    <div class="page-container">
      <div class="page-header">
        <h1>✨ {{ t('vanityWallet.title') }}</h1>
        <p class="subtitle">
          {{ t('vanityWallet.subtitle') }}
        </p>
      </div>
      ...
      <div class="generator-card">
        <div class="settings-section">
            ...
            <div class="form-group">
              <label class="checkbox-label">
                <input
                  ...
                  :disabled="generating"
                >
                {{ t('vanityWallet.prefixLabel') }}
              </label>
              <input
                v-model="prefixPattern"
                type="text"
                :placeholder="t('vanityWallet.prefixLabel')"
            ...
            <div class="form-group">
              <label class="checkbox-label">
                <input
                  ...
                  :disabled="generating"
                >
                {{ t('vanityWallet.suffixLabel') }}
              </label>
              <input
                v-model="suffixPattern"
                type="text"
                :placeholder="t('vanityWallet.suffixLabel')"
          ...
          >
            <span v-if="!generating">🚀 {{ t('vanityWallet.generate') }}</span>
            <span v-else>⏳ {{ t('vanityWallet.generating') }}</span>
          </button>
        ...
        >
          ...
          <div class="stat-item">
            ...
            <div class="stat-value success">
              {{ wallets.length }} / {{ targetCount }}
            </div>
          </div>
          <div class="stat-item">
            <div class="stat-label">
              {{ t('vanityWallet.generating') }}
            </div>
            <div class="stat-value">
        ...
        >
          <div class="results-header">
            <h3>{{ t('vanityWallet.foundLabel') }} ({{ wallets.length }})</h3>
          ...
          <div class="wallets-list">
            ...
            >
              <div class="wallet-header">
                ...
                <div class="wallet-match">
                  <span
                    v-if="matchPrefix"
                    class="match-badge prefix"
                  >
                    {{ t('vanityWallet.prefixLabel') }}: {{ extractMatchedPrefix(wallet.address) }}
                  </span>
                  <span
                    v-if="matchSuffix"
                    class="match-badge suffix"
                  >
                    {{ t('vanityWallet.suffixLabel') }}: {{ extractMatchedSuffix(wallet.address) }}
                  </span>
                </div>
              </div>
              <div class="wallet-info">
                <div class="info-row">
                  ...
                  <span
                    class="value address"
                    v-html="highlightAddress(wallet.address)"
                  />
                  <button
        ...
        >
          ...
          <p>{{ t('vanityWallet.noAddresses') }}</p>
          <p class="empty-hint">
            {{ t('vanityWallet.generate') }}
          </p>
        </div>
      </div>
      ...
      <div class="info-section">
        <div class="info-card">
          <h4>{{ t('qrCode.usageInfo') }}</h4>
          <p>{{ t('vanityWallet.subtitle') }}</p>
        </div>

        <div class="info-card warning">
          <h4>{{ t('vanityWallet.securityWarning') }}</h4>
          <p class="warning-text">
            {{ t('vanityWallet.securityNote') }}
          </p>
        </div>
      </div>
    </div>
...
// SEO Configuration
const pageSEO = getPageSEO('vanityWallet')
useSEO({
  title: pageSEO.title,
...
function matchesPattern(address) {
  ...
  const addrWithoutPrefix = addr.substring(2) // Remove "0x"
  ...
  if (matchPrefix.value) {
    ...
    const prefix = caseSensitive.value ? address.substring(2, 2 + pattern.length) : addrWithoutPrefix.substring(0, pattern.length)
  ...
  if (matchSuffix.value) {
    ...
    const suffix = caseSensitive.value
      ? address.substring(address.length - pattern.length)
...
async function startGeneration() {
  ...
  wallets.value = [] // Clear previous results
  ...
  try {
    while (wallets.value.length < targetCount.value && !shouldStop.value) {
      ...
      for (let i = 0; i < batchSize; i++) {
        ...
        if (matchesPattern(wallet.address)) {
          ...
          if (wallets.value.length >= targetCount.value) break
        }
      }
    ...
    if (wallets.value.length >= targetCount.value) {
...
function exportAsJSON() {
  ...
  downloadFile(data, `vanity_wallets_${Date.now()}.json`, 'application/json')
}
...
function exportAsCSV() {
  ...
  downloadFile(csv, `vanity_wallets_${Date.now()}.csv`, 'text/csv')
}
...
function downloadQRCode() {
  ...
  try {
    const link = document.createElement('a')
    link.download = `vanity_wallet_qrcode_${Date.now()}.png`
